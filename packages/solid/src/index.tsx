...
import { getSource, getUrl, type ImageProps as CreateImageProps, type SrcImageProps } from '@vite-img/core';

export type ImageProps = CreateImageProps<ComponentProps<'img'>>;

export default function Image(props: ImageProps) {
    const [srcProps, rest] = splitProps(props, ['src', 'width', 'height', 'placeholder']);
    ...
    return (
        <img
            ...
            srcSet={sizes.map((size) => `${getUrl({ src: src().src, w: size, q: quality() })} ${size}w`).join(', ')}
            width={src().width}
            height={src().height}
            style={
